...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
MMXTestTire::MMXTestTire(const std::string& name) : ChTire(name), m_use_contact_mesh(false), m_trimesh(nullptr) {}

MMXTestTire::~MMXTestTire() {}

// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void MMXTestTire::SetMeshFilename(const std::string& mesh_file, double sweep_sphere_radius) {
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void MMXTestTire::Initialize(std::shared_ptr<ChWheel> wheel) {
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void MMXTestTire::AddVisualizationAssets(VisualizationType vis) {
    ...
    auto m_sph_shape = chrono_types::make_shared<ChSphereShape>();
    m_sph_shape->GetSphereGeometry().rad = GetRadius();
    m_wheel->GetSpindle()->AddAsset(m_sph_shape);
...
void MMXTestTire::RemoveVisualizationAssets() {
    // Make sure we only remove the assets added by MMXTestTire::AddVisualizationAssets.
...
TerrainForce MMXTestTire::GetTireForce() const {
    // A MMXTestTire always returns zero force and moment since tire forces are automatically applied
...
TerrainForce MMXTestTire::ReportTireForce(ChTerrain* terrain) const {
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
unsigned int MMXTestTire::GetNumVertices() const {
    assert(m_use_contact_mesh);
    return static_cast<unsigned int>(m_trimesh->getCoordsVertices().size());
}

unsigned int MMXTestTire::GetNumTriangles() const {
    assert(m_use_contact_mesh);
    return static_cast<unsigned int>(m_trimesh->getIndicesVertexes().size());
}

const std::vector<ChVector<int>>& MMXTestTire::GetMeshConnectivity() const {
    assert(m_use_contact_mesh);
    return m_trimesh->getIndicesVertexes();
}

const std::vector<ChVector<>>& MMXTestTire::GetMeshVertices() const {
    assert(m_use_contact_mesh);
    return m_trimesh->getCoordsVertices();
}

const std::vector<ChVector<>>& MMXTestTire::GetMeshNormals() const {
    assert(m_use_contact_mesh);
    return m_trimesh->getCoordsNormals();
}

void MMXTestTire::GetMeshVertexStates(std::vector<ChVector<>>& pos, std::vector<ChVector<>>& vel) const {
    assert(m_use_contact_mesh);
